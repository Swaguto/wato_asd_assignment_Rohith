  ...
  double robot_theta)
{
  ...
  for (int j = 0; j < local_h; ++j) {
    for (int i = 0; i < local_w; ++i) {
      ...
      if (occ_val < 0) {
        // Unknown cell: keep whatever the global map already knows
        continue;
      }

      // Center of the local cell, expressed in the robot (lidar) frame
      const double lx = local_origin_x + (i + 0.5) * local_res;
      const double ly = local_origin_y + (j + 0.5) * local_res;

      // Rotate and translate into the world frame using the robot's pose
      ...
      if (occ_val == 0) {
        // Seen free: reveal the cell if it was never observed before, but
        // never erase an obstacle that was previously detected
        if (global_val < 0) {
          global_val = 0;
        }
      } else if (occ_val >= 100) {
        // Occupied: the strongest observation wins and persists in memory even
        // after the robot has driven past the obstacle
        global_val = 100;
      }
      // Inflation values (1-99) are ignored: they are redrawn directly on the
      // global grid from the occupied cells below, which keeps them accurate
      // instead of smearing rotated local cells onto the coarser grid
    }
  }
  ...
  for (const auto& [cell_x, cell_y] : occupied_cells) {
    ...
    for (int y = min_y; y <= max_y; ++y) {
      for (int x = min_x; x <= max_x; ++x) {
        ...
        // Cost falls linearly from 100 at the obstacle to 0 at the radius edge.
        // Cells at the radius edge are left untouched: revealing them as free
        // would let unreachable pockets (e.g. a box interior) masquerade as
        // reachable open space.
