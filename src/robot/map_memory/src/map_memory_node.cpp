...
MapMemoryNode::MapMemoryNode()
  ...
  // Publish a map immediately so the planner never has to wait for the first
  // fusion to happen before it can plan
  timerCallback();
...
void MapMemoryNode::maybeFuse() {
  ...
  // Skip the fusion unless the robot has travelled far enough, or this is the
  // first observation we have ever received
  if (have_last_fuse_) {
