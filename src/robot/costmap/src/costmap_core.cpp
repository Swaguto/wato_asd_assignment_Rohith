...
void CostmapCore::updateCostmap(const sensor_msgs::msg::LaserScan::SharedPtr scan) {
  // A new scan supersedes everything the local window knew before: cells are
  // unknown (-1) again until the scan reveals them
  ...
  for (size_t i = 0; i < scan->ranges.size(); ++i, angle += scan->angle_increment) {
    ...
    // Ignore readings that are out of range or are noise
    if (!std::isfinite(range) || range < scan->range_min || range > scan->range_max) {
      continue;
    }

    // The scan is taken in the lidar frame, so polar -> Cartesian is direct
    ...
    // Reveal the cells along the ray up to the hit as free space. Cells that
    // are already occupied or inflated are never downgraded.
