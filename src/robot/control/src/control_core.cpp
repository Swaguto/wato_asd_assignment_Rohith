  ...
  double robot_theta)
{
  ...
  // Nothing to follow (no path, or already at the end): hold still
  ...
  // Steering angle: heading error between the robot and the lookahead point
  ...
  // When the turn is sharper than we can handle, rotate in place instead of
  // driving sideways into obstacles
...
size_t ControlCore::findLookaheadIndex(size_t closest_index, double robot_x, double robot_y) const {
  // Advance along the path until we find a point at least lookahead_distance
  // away from the robot
  ...
  // The remaining path is shorter than the lookahead distance: aim for the end
